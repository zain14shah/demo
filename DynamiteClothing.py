...
class DynamiteclothingSpider(CrawlSpider):
    ...
    def parse_check_login(self, response):
        if response.css('[href*="profile.jsp"]::attr(href)').extract_first():
            yield response.follow('us?li=true',
                                  callback=self.parse)
        else:
    ...
    def product_price(self, response):
        return response.css(
            '.prodPricePDP::text, .salePrice::text'
        ).re_first('\S+')

    def product_sale_price(self, response):
    ...
    def product_img_urls(self, response):
        return response.css('a::attr(data-zoom)').extract()

    def parse_product(self, response):
        product = Product()
        product['name'] = self.product_name(response)
        product['id'] = self.product_id(response)
        product['price'] = self.product_price(response)
        product['sale_price'] = self.product_sale_price(response)
        product['colors'] = self.product_colors(response)
        product['sizes'] = self.product_sizes(response)
        product['description'] = self.product_description(response)
        product['details'] = self.product_details(response)
        product['img_urls'] = self.product_img_urls(response)
