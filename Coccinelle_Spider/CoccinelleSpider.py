...
class CoccinelleSpider(CrawlSpider):
    ...
    rules = (
        Rule(LinkExtractor(restrict_css=homepage_urls_css,
                           tags=homepage_tags_css)),
        Rule(LinkExtractor(restrict_css=product_urls_css),
    ...
    def product_code(self, response):
        code = response.css(
            'div.b-product_master_id::text'
        ).extract_first().replace('Code:', '').strip()
    ...
    def product_colors(self, response):
        return response.css(
            'a.b-swatches_color-link::attr(title)'
        ).extract() or ['N/A']
    ...
    def product_description(self, response):
        return response.css(
            'div.b-product_long_description::text'
        ).extract_first().strip()

    def product_material(self, response):
        return response.css(
            'div.b-product_material::text'
        ).extract_first().strip()

    def product_care_details(self, response):
        return response.css(
    ...
    def parse_product_details(self, response):
        ...
        product['code'] = self.product_code(response)
        product['sizes'] = self.product_sizes(response)
        product['colors'] = self.product_colors(response)
        product['price'] = self.product_price(response)
        product['description'] = self.product_description(response)
        product['material'] = self.product_material(response)
        product['care_details'] = self.product_care_details(response)
